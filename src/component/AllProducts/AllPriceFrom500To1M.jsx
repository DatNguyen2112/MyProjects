...
function AllPriceFrom500To1M({ user }) {
  ...
  const fetchPriceLow500 = data.filter((item) => {
    return item.price > 500000 && item.price < 1000000;
  });

  console.log(fetchPriceLow500);
  ...
  const filterFunction = (text) => {
    if (fetchPriceLow500.length > 1) {
      const filter = fetchPriceLow500.filter(
        (product) => product.categorys === text
      );
  ...
  return (
    <div>
      ...
      <div className="SignIn__title">
        <h3 className="SignIn__text">Sản phẩm từ 500 nghìn đến 1 triệu</h3>
      </div>
      ...
      <div className="AllProducts__option">
        ...
        <div className="AllProducts__option-products">
          ...
          {filteredProducts.length < 1 && (
            <>
              {" "}
              {fetchPriceLow500.length > 0 && (
                <div className="AllProducts__option-AllProduct">
                  {fetchPriceLow500.map((product) => {
                    return (
              ...
              {fetchPriceLow500.length < 1 && <div>Loading...</div>}{" "}
            </>
          )}{" "}
        </div>
      </div>
...
export default AllPriceFrom500To1M;
