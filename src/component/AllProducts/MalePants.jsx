...
function MalePants({user}) {
    ...
    console.log(fetchShirt)

    const fetchMaleShirt = fetchShirt.filter((item) => {
        return item.categorys === 'Quần'
    })

    console.log(fetchMaleShirt)
    ...
    const filterFunction = (text) => {
        if (fetchShirt.length > 1) {
            const filter = fetchMaleShirt.filter((shirt) => shirt.categorysItem === text);
            ...
            <div className='AllProducts__option'>
                ...
                <div className='AllProducts__option-products'>
                    ...
                    filteredProducts.length < 1 && (
                        <> {
                            fetchMaleShirt.length > 0 && (
                                <div className='AllProducts__option-AllProduct'>
                                    {
                                    fetchMaleShirt.map((product) => {
                                        return (
                                            <Link to={
                                                `/ProductDetail/${
                                                    product.ID
